...
const TopNavbar: React.FC<TopNavbarProps> = ({ user, onLogout }) => {
  ...
  const { currentLanguage, setCurrentLanguage, availableLanguages, isLoadingLanguages, translate } = useLanguage();
  ...
  const navItems = [
    ...
    { id: 'progress', labelKey: 'progress', icon: ChartBarIcon, path: PATHS.PROGRESS },
    { id: 'documents', labelKey: 'documents', icon: DocumentIcon, path: PATHS.DOCUMENTS },
    { id: 'about', labelKey: 'about', icon: InformationCircleIcon, path: PATHS.ABOUT_US },
    { id: 'contact', labelKey: 'contact', icon: PhoneIcon, path: PATHS.CONTACT },
  ];

  const baseLinkClasses = "flex items-center space-x-2 px-3 py-2 rounded-md text-sm font-medium transition-colors duration-150";
  const inactiveLinkClasses = "text-gray-300 hover:bg-primary-dark hover:text-white";
      ...
      <div className="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          <div className="flex items-center">
            <Link to={PATHS.DASHBOARD_ROOT} className="flex-shrink-0 flex items-center space-x-2">
          ...
          <div className="hidden md:flex items-center space-x-1 lg:space-x-2"> {/* Reduced space for more items */}
            {navItems.map((item) => (
              ...
              >
                ...
                <span>{labels[item.labelKey as keyof typeof labels] || item.labelKey}</span>
              </NavLink>
            ))}
          </div>
          <div className="hidden md:flex items-center space-x-3">
             <div className="relative">
                <button 
                    ...
                    onClick={(e) => (e.currentTarget.nextElementSibling as HTMLSelectElement)?.focus()} // Hack to open select on button click visually
                >
                    <GlobeAltIcon className="w-5 h-5 mr-1" />
                    <span className="hidden lg:inline">{availableLanguages.find(l => l.code === currentLanguage)?.name || currentLanguage.toUpperCase()}</span>
            ...
            <div className="relative">
              ...
              >
                ...
                <UserIcon className="h-8 w-8 rounded-full text-primary-light bg-slate-700 p-1" />
                 <span className="ml-2 text-sm font-medium text-gray-300 hidden lg:block">{user.name || user.email}</span>
                <ChevronDownIcon className={`ml-1 h-5 w-5 text-gray-400 transform transition-transform duration-200 ${isUserMenuOpen ? 'rotate-180': ''} hidden lg:block`} />
              </button>
              {isUserMenuOpen && (
                <div
                ...
                >
                  <div className="px-4 py-3 border-b border-gray-200">
                    ...
                    <p className="text-sm font-medium text-darktext truncate" title={user.email}>
                      {user.email}
                    </p>
                  </div>
                  <button
          ...
          <div className="-mr-2 flex md:hidden">
            ...
            >
              ...
              {isUserMenuOpen ? <XCircleIcon className="block h-6 w-6" /> : <UserIcon className="block h-6 w-6" />}
            </button>
          </div>
        </div>
      </div>
          ...
          <div className="md:hidden" id="mobile-menu">
            <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
               {navItems.map((item) => (
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `block ${baseLinkClasses} ${isActive ? activeLinkClasses : inactiveLinkClasses}`
                  }
                >
                  <item.icon className="w-5 h-5" />
                  <span>{labels[item.labelKey as keyof typeof labels] || item.labelKey}</span>
            ...
            <div className="pt-4 pb-3 border-t border-slate-700">
              <div className="flex items-center px-5">
                ...
                <div className="ml-3">
                  <div className="text-base font-medium leading-none text-white">{user.name || user.email.split('@')[0]}</div>
              ...
              <div className="mt-3 px-2 space-y-1">
                ...
                >
                  <LogoutIcon className="w-5 h-5 inline mr-2" />{labels.logout}
                </button>
              </div>
            </div>
          </div>
