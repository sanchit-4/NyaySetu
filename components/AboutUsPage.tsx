...
const AboutUsPage: React.FC = () => {
  ...
  const generateAudiobooks = async () => {
    ...
    try {
      ...
      const processedStories: Story[] = sampleStorybooksData.map(
        (item, index) => ({
          id: `story-${index + 1}`,
          story: item.story,
  ...
  const translateAllStories = async (
    storiesToTranslate: Story[],
    targetLanguage: string
  ) => {
      ...
      <div className="bg-white rounded-2xl shadow-xl p-8">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-gray-600">
            Generate, translate, and listen to stories in multiple languages
          </p>
        </div>
        ...
        {stories.length > 0 && (
          <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
            <p className="text-blue-700 flex items-center gap-2">
              <span>ℹ️</span>
              Currently displaying stories in:{" "}
              <strong>
          ...
          {stories.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-400">
                Click "Generate Audiobooks" to load your stories!
              </p>
            </div>
          ) : (
            stories.map((story) => (
                ...
                <div className="flex justify-between items-start mb-4">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="px-3 py-1 bg-white rounded-full text-sm font-medium text-gray-700">
                      {
                        languages.find(
                          (lang) => lang.code === story.currentLanguage
                        )?.name
                      }
                    </span>
                  </div>
        ...
        {stories.length > 0 && (
          <div className="text-center mt-8 pt-6 border-t border-gray-200">
            <p className="text-gray-500 text-sm">
              📁 Audio files should be located in:{" "}
              <code className="bg-gray-100 px-2 py-1 rounded">jhh</code>
            </p>
            <p className="text-gray-400 text-xs mt-2">
              English and Hindi have dedicated audio files. Other languages use
              English audio with translated text.
            </p>
          </div>
        )}
      </div>
